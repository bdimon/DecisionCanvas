...
app.post('/api/analyze', async (req, res) => {
  try {
    ...
    if (!option1 || !option2) {
      return res.status(400).json({ error: language === 'en' ? 'Please provide both options.' : 'Пожалуйста, укажите оба варианта решения.' });
    }
    ...
    if (!ai) {
      ...
      const localData = generateLocalAnalysis(option1, option2, context, language);
      return res.json({
        usingAI: false,
        data: localData,
        message: language === 'en' ? 'Generated using built-in decision intelligence engine.' : 'Использован встроенный экспертный генератор анализа.'
      });
    }
...
Сформируй анализ строго в соответствии со схемой JSON.`;
    ...
    const fallbackData = generateLocalAnalysis(option1, option2, context, language);
    return res.json({
      usingAI: false,
      data: fallbackData,
      note: language === 'en' ? 'Generated by built-in analytical engine' : 'Сгенерировано встроенным модулем аналитического анализа'
    });
  } catch (error: any) {
    console.log('Notice: Fallback engine activated for analysis.');
    const { option1, option2, context, language = 'ru' } = req.body || {};
    const fallbackData = generateLocalAnalysis(option1 || (language === 'en' ? 'Option A' : 'Вариант 1'), option2 || (language === 'en' ? 'Option B' : 'Вариант 2'), context, language);
    return res.json({
      usingAI: false,
      data: fallbackData,
      note: language === 'en' ? 'Generated by built-in expert engine' : 'Сгенерировано встроенным модулем экспертного анализа'
    });
  }
});
